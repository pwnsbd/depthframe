...
let nextRequestId = 1;
const pending = new Map<number, { resolve: (result: DepthResult) => void; reject: (error: Error) => void }>();
...
function getWorker(): Worker {
	...
	worker = new Worker(new URL("./depthWorker.ts", import.meta.url), {
		type: "module",
	});
	...
	worker.onerror = (event) => {
		setState({ status: "error", error: event.message || "The depth worker crashed." });
	};
...
// Runs depth estimation for one image. The worker keeps the loaded pipeline
// cached across calls, so only the first call in a session pays the
// download/load cost — subsequent calls go straight to inference.
export function runDepth(imageBitmap: ImageBitmap): Promise<DepthResult> {
...
// Transformers.js pipelines can't be aborted mid-inference, so the only way
// to actually stop a running depth request is to kill the worker thread that's
// running it. The next runDepth() call spins up a fresh worker and reloads
// the pipeline (a cache hit in the browser's model cache, not a re-download).
export function cancelDepth(): void {
	...
	for (const request of pending.values()) {
		request.reject(new DepthCancelledError());
	}
	pending.clear();
